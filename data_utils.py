...
def one_hot(labels, num_classes, pos_label=1.0, neg_label=0.0, scope=None):
    with tf.name_scope(scope, 'one_hot', [labels]):
        batch_size = labels.get_shape()[0]
        indices = tf.expand_dims(tf.range(0, batch_size), 1)
        labels = tf.cast(tf.expand_dims(labels, 1), indices.dtype)
        concatenated_labels = tf.concat(axis=1, values=[indices, labels])
        onehot_labels = tf.sparse_to_dense(
            concatenated_labels, tf.stack([batch_size, num_classes]), pos_label, neg_label)
...
def load_cifar100_dataset(dirname, labels='fine', transpose_permutation=(0,2,3,1)):
    ...
    Args:
        dirname - file location of extracted cifar100 tarball, or cifar10 tarball
...
def load_dataset(dataset_name):
    ...
    #    return load_cifar100_dataset("data/cifar-10-python", labels='fine')
